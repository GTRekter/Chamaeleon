...
namespace ThreadSample
{
    ...
    public static class IOFunctions
    {
        ...
        /// <param name="maxLength">Maximum length of string to get</param>
        /// <returns>String value entered by user</returns>
        public static string GetStringFromUser(string prompt, int minLength, int maxLength)
        {
            // Remember original console color so we can set it back when exiting
            ConsoleColor originalColor = Console.ForegroundColor;
            ...
            try
            {
                ...
                {
                    Console.ForegroundColor = ErrorColor;
                    Console.WriteLine("Invalid length.  Value must be between {0} and {1} characters.", minLength, maxLength);
                    // Call this function recursively until a valid pasword is given
                    input = GetStringFromUser(prompt, minLength, maxLength);
                }
        ...
        /// <param name="prompt">Prompt to display to the user</param>
        /// <param name="minLength">Minimum length of password</param>
        /// <param name="maxLength">Maximum length of password to get (0 = unlimited)</param>
        /// <returns>String value entered by user</returns>
        public static string GetPasswordFromUser(string prompt, int minLength, int maxLength)
        {
            // Remember original console color so we can set it back when exiting
            ConsoleColor originalColor = Console.ForegroundColor;
            ...
            try
            {
                ...
                if ((input.Length < minLength) || (input.Length > maxLength))
                {
                    ...
                    Console.WriteLine("Invalid length.  Value must be between {0} and {1} characters.", minLength, maxLength);
                    // Call this function recursively until a valid pasword is given
                    input = GetPasswordFromUser(prompt, minLength, maxLength);
                }
		...
		/// <remarks>This function will not exit until the user gives a valid input</remarks>
		public static decimal GetDecimalFromUser(string prompt, decimal lowValue, decimal highValue)
		{
			// Remember original console color so we can set it back when exiting
			ConsoleColor originalColor = Console.ForegroundColor;
